...
export const uploadProject = async (values: z.infer<typeof projectSchema>, userid: string, pathname: string) => {
  if (!values) {
...
export const updateProject = async (values: z.infer<typeof projectSchema>, id: string) => {
  try {
...
export const getProjectsByUserId = async (id: string) => {
  try {
    ...
    if (!projects) {
      throw new Response("No projects found or there was an error fetching projects");
    }
